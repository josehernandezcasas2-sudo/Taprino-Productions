...
export async function getPublicEpisodes() {
  ...
  const [episodesResult, seriesResult] = await Promise.all([
    supabase
      .from('episodes')
      .select('id, title, description, tier, genre, main_genre, content_type, series_id, season, series_order, artist, runtime, rating, is_original, bonus_parent_type, bonus_parent_id, video_type, trailer_src, hero_image, poster, thumbnail, featured, available_from, available_until, ads_enabled')
      .eq('status', 'approved')
