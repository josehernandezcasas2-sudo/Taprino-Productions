...
// isAdmin, isCreator }. `email` is fetched here (not via a separate call)
// specifically so admin-action endpoints can pass it straight to
// lib/auditLog.js's recordAudit without an extra Clerk round-trip.
export async function getRoleContext(req) {
  const { userId } = getAuth(req);
  if (!userId) {
    return { userId: null, role: null, email: null, isAdmin: false, isCreator: false };
  }
  const client = await clerkClient();
  const user = await client.users.getUser(userId);
  ...
  return {
    userId,
    role,
    email: primary ? primary.emailAddress : (user.emailAddresses[0] ? user.emailAddresses[0].emailAddress : null),
    isAdmin: role === 'admin',
