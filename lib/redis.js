...
// Strips accidental surrounding quotes — a common mistake when copying a
// value into Vercel's env var UI from somewhere that displayed it quoted
// (e.g. "https://foo.upstash.io" saved literally, quotes and all). That
// produces a URL string that looks almost right but fails to parse, with
// an error message that's easy to misread as "invalid database" rather
// than "extra characters in the value."
...
const BASE = stripQuotes(process.env.UPSTASH_REDIS_REST_URL);
...
/* ===================================================================
   Generic read-through cache for expensive, rarely-changing queries
   (getPublicEpisodes, getAllSeries) that were previously re-run against
   Supabase on every single page load — 47 call sites for
   getPublicEpisodes alone, with zero caching anywhere.

   A large JSON value (a few hundred episodes serialized) can't safely
   go through redisFetch above: that helper puts every argument in the
   URL path, and a payload that size risks hitting URL length limits.
   Per Upstash's own REST API docs, a JSON/binary value belongs in a POST
   request body instead, with any additional command arguments (like the
   TTL) passed as query params — https://upstash.com/docs/redis/features/restapi.
   GET is fine as-is since it only ever needs the key, never the value.

   Fails open in both directions: unconfigured or erroring Redis falls
   back to calling fetcher() directly rather than breaking the page, and
   a write failure still returns the freshly-fetched data even though it
   didn't get cached that time.
   =================================================================== */

async function redisSetJson(key, value, ttlSeconds) {
  const url = `${BASE}/set/${encodeURIComponent(key)}${ttlSeconds ? `?EX=${ttlSeconds}` : ''}`;
...
export async function getOrSetCached(key, ttlSeconds, fetcher) {
  if (!isRedisConfigured()) return fetcher();

  try {
    const cached = await redisGetJson(key);
    if (cached !== null) return cached;
  } catch (err) {
    console.error(`getOrSetCached read error (${key}):`, err.message);
    // Fall through to fetching fresh — a cache read failure should never
  ...
  try {
    ...
    // Non-fatal — fresh is still returned below even if it didn't get
    // cached this time, so the caller never sees this failure.
    console.error(`getOrSetCached write error (${key}):`, err.message);
  }
  return fresh;
}

// Called by admin write paths (create/edit/delete episode) so a change
// shows up immediately instead of waiting out the TTL — see the call
// sites in pages/api/admin/*.js. Safe to call even if the key was never
// cached (DEL on a missing key is a normal no-op, not an error).
export async function invalidateCache(key) {
  if (!isRedisConfigured()) return;
  try {
    await redisFetch(['del', key]);
  } catch (err) {
    console.error(`invalidateCache error (${key}):`, err.message);
  }
}
...
/* ===================================================================
   Ad impression tracking, for the advertiser's own "performance over
   time" view — exact same daily-hash-bucket shape as episode view
   tracking above (ad_impressions:YYYY-MM-DD instead of
   episode_views:YYYY-MM-DD), just keyed by house_ads.id instead of an
   episode id.
   =================================================================== */

export async function recordDailyAdImpression(adId) {
  if (!isRedisConfigured() || !adId) return;
  const key = `ad_impressions:${dayKey()}`;
  try {
    await redisFetch(['hincrby', key, adId, '1']);
    await redisFetch(['expire', key, String(DAY_TTL_SECONDS)]);
  } catch (err) {
    console.error('recordDailyAdImpression error:', err.message);
  }
}

// Same shape as getDailyViews — [{ date, counts: { adId: n } }], oldest
// first, missing days come back empty so a chart drawn from this has no
// phantom gaps.
export async function getDailyAdImpressions(days = 14) {
  if (!isRedisConfigured()) return [];
  const today = new Date();
  const keys = [];
  for (let i = days - 1; i >= 0; i--) {
  ...
  try {
    return await Promise.all(
      keys.map(async (k) => {
        try {
          const data = await redisFetch(['hgetall', `ad_impressions:${k}`]);
          const flat = data.result || [];
          const counts = {};
          for (let i = 0; i < flat.length; i += 2) counts[flat[i]] = Number(flat[i + 1]);
          return { date: k, counts };
    ...
    console.error('getDailyAdImpressions error:', err.message);
    return [];
  }
}
...
export async function recordWatchSeconds(episodeId, seconds) {
  ...
  const rounded = String(Math.round(seconds));
  try {
    await redisFetch(['hincrby', key, episodeId, rounded]);
    await redisFetch(['expire', key, String(DAY_TTL_SECONDS)]);
    // Lifetime running total, same shape as the episode_views zset below —
    // this is what makes "average watch time" possible as a lifetime
    // stat consistent with the lifetime Views column, rather than having
    // to reconstruct a lifetime figure by summing daily buckets (which
    // would also mean an ever-growing number of Redis calls as the site
    // accumulates history).
    await redisFetch(['zincrby', 'episode_watch_seconds', rounded, episodeId]);
  } catch (err) {
    console.error('recordWatchSeconds error:', err.message);
  }
}

// Returns { episodeId: totalSecondsWatched } — lifetime, mirroring
// getViewCounts(). Only reflects watch time recorded after this lifetime
// tracker was added; it doesn't retroactively backfill from the daily
// buckets that existed before it.
export async function getWatchSecondsTotals() {
  if (!isRedisConfigured()) return {};
  try {
    const data = await redisFetch(['zrange', 'episode_watch_seconds', '0', '-1', 'WITHSCORES']);
    const flat = data.result || [];
    const totals = {};
    for (let i = 0; i < flat.length; i += 2) {
      totals[flat[i]] = Number(flat[i + 1]);
    }
    return totals;
  } catch (err) {
    console.error('getWatchSecondsTotals error:', err.message);
    return {};
  }
}
