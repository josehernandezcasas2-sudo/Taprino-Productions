...
// Cloudflare Stream auto-generates a thumbnail JPG at this fixed path for
// every video — no separate API call or admin-uploaded artwork needed.
// Used for user-posted videos (lib/posts.js), which don't go through the
// episode poster/thumbnail upload flow.
export function cloudflareThumbnailUrl(uid) {
  const code = process.env.CLOUDFLARE_STREAM_CUSTOMER_CODE;
  if (!code || !uid) return null;
...
/* ===================================================================
   ...
   URL by someone who did) can play or download Studio Tapa + content
