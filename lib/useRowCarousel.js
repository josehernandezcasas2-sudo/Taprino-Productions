import { useRef, useEffect } from 'react';

// Matches .row-card's width (190px) + .cat-row-track's gap (1.4rem ≈ 22px)
// in styles/globals.css — scrolling by this amount lines up with actual
// card boundaries instead of an arbitrary pixel jump.
const CARD_STEP_PX = 212;

// Shared by every horizontally-scrolling row (GenreRow, ContinueWatchingRow,
// and any future one) so the loop-around behavior stays identical
// everywhere rather than drifting between two hand-copied versions.
export function useRowCarousel() {
  const trackRef = useRef(null);

  useEffect(() => {
    const track = trackRef.current;
    if (!track) return;

    // A normal mouse has no horizontal scroll input at all — spinning the
    // wheel over a horizontal row does nothing today, which reads as
    // broken even though the row itself works fine via drag/touch/arrows.
    // This converts an ordinary vertical wheel scroll into horizontal
    // movement, but ONLY while the cursor is over this specific row
    // (native browser event scoping — nothing needed to "contain" it
    // manually) and ONLY when the gesture is predominantly vertical.
    // A trackpad's genuine horizontal swipe (deltaX already dominant) is
    // left completely alone, so two-finger horizontal scrolling keeps
    // working exactly as it already does.
    function handleWheel(e) {
      if (Math.abs(e.deltaY) <= Math.abs(e.deltaX)) return;
      e.preventDefault();
      track.scrollLeft += e.deltaY;
    }

    // { passive: false } is required — preventDefault() on a wheel event
    // is silently ignored (and Chrome logs a warning) on a passive
    // listener, which is the default for wheel/touch listeners added any
    // other way.
    track.addEventListener('wheel', handleWheel, { passive: false });
    return () => track.removeEventListener('wheel', handleWheel);
  }, []);

  function scroll(direction) {
    const track = trackRef.current;
    if (!track) return;
    const visibleCards = Math.max(1, Math.floor(track.clientWidth / CARD_STEP_PX));
    const amount = CARD_STEP_PX * visibleCards;

    if (direction === 'next') {
      const atEnd = track.scrollLeft + track.clientWidth >= track.scrollWidth - 4;
      track.scrollTo({ left: atEnd ? 0 : track.scrollLeft + amount, behavior: 'smooth' });
    } else {
      const atStart = track.scrollLeft <= 4;
      track.scrollTo({ left: atStart ? track.scrollWidth : track.scrollLeft - amount, behavior: 'smooth' });
    }
  }

  return { trackRef, scroll };
}
