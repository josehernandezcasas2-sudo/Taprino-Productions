...
const EPISODE_FIELDS = 'id, title, description, tier, genre, artist, runtime, video_type, src, poster, thumbnail, rating, release_year, ads_enabled';
...
export async function addEpisodeToSchedule(episodeId) {
  ...
  if (episode.tier !== 'free') throw new Error(`Only free-tier episodes can go on the channel — ${SITE.premiumTier} content stays behind the paywall everywhere, including here.`);
...
export async function getChannelState(atTime = new Date()) {
  ...
  return {
    ...
    program: {
      ...
      ...current.episode,
      releaseYear: current.episode.release_year
    },
    next: {
      title: next.episode.title,
      genre: next.episode.genre,
      runtime: next.episode.runtime,
      tier: next.episode.tier,
      scheduleId: next.id
    }
  };
}
