...
export async function getPodcastShowEpisodes(seriesId, isSubscriber) {
  ...
  const { data, error } = await supabase
    .from('episodes')
    .select('id, title, description, tier, runtime, season, series_order, src, audio_url, poster, thumbnail, created_at, ad_break_seconds, ads_enabled')
  ...
  return data.map((row) => {
    ...
    return {
      ...
      adBreakSeconds: row.ad_break_seconds,
