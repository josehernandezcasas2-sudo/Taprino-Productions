...
//
// isSubscriber (Studio Tapa + access) is now true for THREE separate
...
//   3. An actual active Stripe subscription — the normal paying-customer
...
export async function getAccountContext(req) {
  ...
  try {
    ...
    const isComped = await isCompedEmail(email);
    const privilegedFreeAccess = isAdmin || isSubAdmin || isComped;
