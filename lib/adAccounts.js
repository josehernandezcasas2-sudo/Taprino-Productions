...
// Only for an ad that's already been through review — increasing budget
// doesn't touch anything admin actually reviewed (creative, targeting,
// the billing rate), so it deliberately doesn't require going back
// through the review queue at all. ad_account_id is checked directly in
// the query (not just assumed from the caller) so this can never
// increase a different advertiser's ad even if a wrong or tampered adId
// were somehow passed in.
export async function increaseAdBudget({ adId, adAccountId, additionalCents }) {
  ...
  const { data: existing, error: fetchError } = await supabase
    .from('house_ads')
    .select('id, review_status, active, budget_total_cents, budget_spent_cents')
    .eq('id', adId)
    .eq('ad_account_id', adAccountId)
    .maybeSingle();
  ...
  if (existing.review_status !== 'approved') {
    throw new Error('Only an already-approved ad\'s budget can be increased this way.');
  }

  const newBudgetTotal = (existing.budget_total_cents || 0) + additionalCents;
  const updates = { budget_total_cents: newBudgetTotal };
  // If the ad had been auto-paused specifically because it ran out of
  // budget (increment_house_ad_impression's own logic — see migration
  // 056), it now has room again and should pick back up automatically;
  // that's the entire point of adding more budget. There's no separate
  // "admin paused this for another reason" flag to worry about
  // conflating this with — active is the only such flag that exists.
  if (!existing.active && existing.budget_spent_cents < newBudgetTotal) {
    updates.active = true;
  }
