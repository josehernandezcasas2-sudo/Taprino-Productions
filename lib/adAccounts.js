...
// Validates a placements array against lib/adPlacements.js's known
// values — the single place this check happens, called from submit,
// edit, and admin review alike, so an invalid value can never slip
// through from any of the three entry points. Empty/null both mean "no
// restriction" and pass straight through unchanged.
function validatePlacements(placements) {
  ...
  const valid = new Set(AD_PLACEMENTS.map((p) => p.value));
  for (const p of placements) {
    if (!valid.has(p)) throw new Error(`Unknown placement: ${p}`);
  }
  return placements;
}
...
export async function submitAdvertiserAd({ adAccountId, title, videoBase64, videoFileName, durationSeconds, clickUrl, budgetTotalCents, targeting, placements }) {
  ...
  const validatedPlacements = validatePlacements(placements);
...
export async function editPendingAd({ adId, adAccountId, title, videoBase64, videoFileName, durationSeconds, clickUrl, budgetTotalCents, placements }) {
  if (!adId || !adAccountId) throw new Error('adId and adAccountId are required.');
  if (budgetTotalCents != null && (!Number.isFinite(budgetTotalCents) || budgetTotalCents <= 0)) {
    throw new Error('Budget must be a positive amount.');
  }
  ...
  if (placements !== undefined) updates.placements = validatePlacements(placements);
  if (videoBase64) {
...
// Every ad account, with aggregate stats — the admin overview this
// entire function exists for. Two queries rather than one complex join:
// fetch every ad account, fetch every advertiser-submitted house ad
// (ad_account_id not null — admin's own house ads are excluded, they're
// not part of this picture), then group the ad counts by account in JS.
// At the scale this platform is at, this is simpler and just as fast as
// a SQL-side aggregate, and easier to read a year from now.
export async function getAllAdAccounts() {
  const supabase = getSupabase();
  const [{ data: accounts, error: accountsError }, { data: ads, error: adsError }] = await Promise.all([
    supabase.from('ad_accounts').select('*').order('created_at', { ascending: false }),
    supabase.from('house_ads').select('ad_account_id, review_status, budget_spent_cents').not('ad_account_id', 'is', null)
  ]);
  if (accountsError) {
    console.error('getAllAdAccounts error:', accountsError.message);
    return [];
  }
  if (adsError) {
    console.error('getAllAdAccounts (ads) error:', adsError.message);
  }

  const statsByAccount = {};
  for (const ad of ads || []) {
    const id = ad.ad_account_id;
    if (!statsByAccount[id]) {
      statsByAccount[id] = { pending: 0, approved: 0, rejected: 0, withdrawn: 0, totalSpentCents: 0 };
    }
    const stats = statsByAccount[id];
    if (stats[ad.review_status] !== undefined) stats[ad.review_status] += 1;
    stats.totalSpentCents += Number(ad.budget_spent_cents) || 0;
  }

  return (accounts || []).map((row) => {
    const account = rowToAdAccount(row);
    const stats = statsByAccount[account.id] || { pending: 0, approved: 0, rejected: 0, withdrawn: 0, totalSpentCents: 0 };
    return { ...account, ...stats };
  });
}

// The suspend/reactivate action from the new admin overview page — the
// only place in the codebase that actually writes ad_accounts.status.
// Every read path that checks account.status !== 'active' (submit-ad,
// buy-credits, the dashboard's suspended banner) was already built
// assuming this could happen; this is what actually makes it possible.
export async function setAdAccountStatus({ adAccountId, status }) {
  if (!adAccountId) throw new Error('adAccountId is required.');
  if (!['active', 'suspended'].includes(status)) {
    throw new Error('status must be active or suspended.');
  }
  const supabase = getSupabase();
  const { data, error } = await supabase.from('ad_accounts').update({ status }).eq('id', adAccountId).select('*').single();
  if (error) throw new Error(error.message);
  return rowToAdAccount(data);
}
...
export async function reviewAdvertiserAd({ adId, decision, adminEmail, title, videoBase64, videoFileName, durationSeconds, clickUrl, costPerImpressionCents, rejectionReason, placements }) {
