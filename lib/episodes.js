...
export async function findEpisode(id) {
  ...
  // a series AND is missing its own poster, thumbnail, or title image).
  let seriesArt = null;
  if (data.series_id && (!data.poster || !data.thumbnail || !data.title_image_url)) {
    const { data: seriesRow } = await supabase.from('series').select('poster, thumbnail, title_image_url').eq('id', data.series_id).maybeSingle();
    seriesArt = seriesRow;
  }
...
function rowToEpisode(row, seriesArt) {
  return {
    ...
    adBreakSeconds: Array.isArray(row.ad_break_seconds) && row.ad_break_seconds.length > 0 ? row.ad_break_seconds : [0],
