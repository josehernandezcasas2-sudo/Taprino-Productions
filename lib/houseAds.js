...
// cumulative range.
export async function pickActiveHouseAd(placement) {
  ...
  // Date-window and placement filtering both happen here rather than in
  // the query itself — the date comparison avoids any timezone mismatch
  // between the DB server and wherever "today" gets computed, and
  // filtering placements in JS avoids Postgres array-containment query
  // syntax for what's a handful of rows at house-ad scale.
  ...
  const ads = (data || []).filter((ad) => {
    ...
    // No placement requested by the caller, or the ad itself has no
    // placements set (eligible everywhere) — either way, don't filter.
    if (placement && ad.placements && ad.placements.length > 0 && !ad.placements.includes(placement)) return false;
    return true;
  });
