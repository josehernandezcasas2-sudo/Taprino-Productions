...
// reaches the browser. { ssr: false } skips that entirely: the component
// only ever renders client-side, after hydration.
...
// The actual submission form — used both as the full page at /creator
// and embedded in a modal from /creator/my-work's "Add bonus content"
...
export default function CreatorSubmissionForm({ allSeries, initialContentType, initialSeriesId, onSubmitted }) {
  ...
  // Pre-fills from whatever the caller passed in — pages/creator.js reads
  // these from its own ?contentType=&seriesId= query params (deep-linked
  // from the "Add bonus content" option before this was a popup); the
  // modal usage passes them directly as props instead, with no query
  // string involved at all.
  useEffect(() => {
    if (initialContentType && CONTENT_TYPES.some((t) => t.value === initialContentType)) {
      setForm((f) => ({
        ...f,
        contentType: initialContentType,
        ...(initialSeriesId ? { seriesId: initialSeriesId } : {})
      }));
    }
    // Only ever meant to apply the caller's initial values once, on mount
    // — not every time the parent happens to re-render with the same props.
  ...
  async function handleSubmit(e) {
    ...
    // Fires right as the upload is *initiated*, not once it completes —
    // matching the page's existing behavior of letting a creator move on
    // immediately while the upload keeps running in the background. For
    // the modal usage this is what closes the popup.
